...
class VideoController extends Controller
{
    ...
    public function codec()
    {
        ...
        //获取状态为 0 的视频
        $list = ShopVideo::where(['status'=>0])->orderBy("id","desc")->get();
        ...
        if($list)
        {
            foreach($list as $k=>$v)
            {
                ...
                //开始转码
                ShopVideo::where(['goods_id'=>$goods_id])->update(['status'=>1]);      //更新转码状态为 1  开始转码
                ...
                ShopVideo::where(['goods_id'=>$goods_id])->update(['status'=>2,'m3u8'=>$m3u8_file]);  //更新转码状态为完成
